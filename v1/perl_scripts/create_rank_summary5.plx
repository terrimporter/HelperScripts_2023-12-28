...
#fix again, problem with species, genus coverage is lower?!?
		...
		if ($line2 =~ /^$query_gi/){
			...
			if ($index1>=0) {
				...
				if ($next==1) {
					$rank =~ s/\;//;
					$genus_maybe = $rank;
					if ($genus_maybe =~ /(aceae$|ales$|Ichthyophonida|mycetes$|Ichthyosporea|mycota$|Fungi$)/) {
					       $index++;
					       $next=0;
				      	}	       
					elsif ($genus_maybe =~ /(species complex|complex)/) {
						$next=1;
					}
					elsif ($genus_maybe =~ /^[A-Z]/) {
						if ($genus_flag==0) {
							$genus = $genus_maybe;
							$next=0;
						}
					...
					if ($rank =~ /\.\.$/) {
						...
						if ($species_maybe =~ /^[a-z]/) {
							@species_maybe = split(/ /,$species_maybe);
							$val = scalar(@species_maybe);
							if ($val==1) {
								$species_maybe =~ s/\.//g;
								if ($species_maybe =~ /(aceae$|ales$|Ichthyophonida|mycetes$|Ichthyosporea|mycota$|Fungi$)/) {
									$index1++;
								}
								elsif ($species_maybe !~ /(other sequences|artificial sequences|construct|fungus|^sp$|mycorrhizae|mycorrhizal samples|endophyte|from|tricholomataceous ant symbiont|pilosum)/) {
									if ($species_flag==0) {
					...
					elsif ($rank =~ /(ales$|Ichthyophonida|ida$)/) {
						if ($order_flag==0) {
							$order = $rank;
							$order_flag=1;
						}
					}
					elsif ($rank =~ /(mycetes$|Ichthyosporea|Coccidia)/) {
						if ($class_flag==0) {
							$class = $rank;
							$class_flag=1;
						}
					}
					elsif ($rank =~ /(mycota$|Apicomplexa|Streptophyta)/) {
						if ($phylum_flag==0) {
							$phylum = $rank;
							$phylum_flag=1;
						}
					}
					elsif ($rank =~ /(Fungi$|Viridiplantae$|Metazoa$)/) {
						if ($kingdom_flag==0) {
							$kingdom = $rank;
							$kingdom_flag=1;
						}
