...
#March 3,2011 modify to work sorty by primer only (not mid)
#Usage: $perl Lepidoptera_part1_sort.plx
#Modified to use perl regex to find exact primer match instead of agrep, because long primers are too long for agrep!
...
open (IN,"<","temp2.txt") || die "Error cannot open file.fasta.midtrimmed:$!\n";				
...
open (TEMP1,">>","temp2.agrep") || die ("Error cannot open .agrep file: $!\n");
...
open (IN2,"<","primer.exact") || die ("Error cannot open primer.exact: $!\n");
...
print "\nPlease enter number of allowed agrep mismatches:\n";
$mismatches = <STDIN>;
chomp $mismatches;

open(IN,"<","temp2.agrep") || die ("Error can't open temp2.agrep to search: $!\n");
@search = <IN>;
close IN;
...
while ($primersequence[$i]) {
	...
	###adjust agrep settings here###
#	@output2 = qx(agrep -i -$mismatches -D1 -I1 "^$pattern" "temp2.agrep");
	...
	#$folder_name = $fname[0];
	$folder_name = $fname;
	unless (-d $folder_name) { #only mkdir if it doesn't already exist	
		mkdir $folder_name;
		push(@folder_names_primer,$folder_name);
	}
			
	$path_for_filename = $folder_name."/".$filename;
...
sub merge_agrep_results_for_same_locus {
	...
	while ($folder_names_primer[$l]) {
		...
		#chdir ("/home/terri/Lepidoptera/second_filter_set/lib_1_2_new/lib_1/");	
		print "array agrep files: @agrep_files\n";#test
		
		while ($agrep_files[$j]) {
#			@current_folder = split(/_/,$current_folder);
			#$folder = pop(@current_folder);
#			$folder = $current_folder[0];
			$outfile_name = "agrep.merged";
...
#			open (OUT,">>",$outfile_name) || die ("Error cannot open .merged file: $!\n");
			...
			chdir ($dir);
			open (OUT,">>",$outfile_name) || die ("Error cannot open .merged file: $!|n");
			print OUT @new_array;
			close (FH);
			$j++;
		}
...
open (IN,"<","agrep.merged") || die "Error opening merged file: $!\n";
@file1 = <IN>;
close IN;
...
open (OUT, ">>", "agrep.merged.dereplicated") || die "Error opening dereplicated file: $!\n";
...
open (FNA,"<","agrep.merged.dereplicated") || die ("Error opening dereplicated file: $!\n");
@fna = <FNA>;
close FNA;
#print "fna array: @fna\n";#test	
...
open (FASTA,"<","agrep.merged.dereplicated") || die ("Error opening dereplicated fasta: $!\n");
@fasta = <FASTA>;
close FASTA;
...
open (QUAL,"<","sorted.qual") || die ("Error opening sorted qual file: $!\n");
@qual = <QUAL>;
close QUAL;
...
$fastaname_sorted_path = "sorted.fasta";
...
#	chdir("/home/terri/Saina/");
unlink("temp1.txt","temp.qual","temp2.txt","temp2.qual","temp2.agrep","agrep.merged","agrep.merged.dereplicated");
#}
