...
my $gi_batch;
my $string="";
my $batchSize=200;##### Edit batch size here #####
			...
			if ($feat_object -> primary_tag eq "CDS") {
				...
				if ($feat_object -> has_tag('gene')) {
					for $value ($feat_object -> get_tag_values('gene')) {
						if ($value =~ /cytb/i) { #double check product
