#!/usr/bin/perl
#Oct.6, 2011 edited to work with gi id only
...
#edit pattern to parse out id number (gi)
...
open (OUT,">>","clone_summary_families.txt") || die ("Error cannot write to outfile:$!\n");
...
open (IN,"<","clone_summary_families.txt") || die ("Error cannot read from clone summary txt: $!\n");
@in = <IN>;
close IN;

open (OUT2,">>","clone_summary_best_families.txt") || die ("Error cannot write to clone summary best txt: $!\n");
...
#		$id_part =~ /\d+\|\*\|(\w{14})/;
		#my @id_part = split(/\|/,$id_part);###edit here to grab the id (gi)###
		$id_current = $id_part;
...
#keep only best results with >xx% ConstrainedNJ bootstrap proportion
...
open (IN,"<","clone_summary_best_families.txt") || die ("Error can't read best family file: $!\n");
@infile = <IN>;
close IN;
...
print "There are $num_keep families with support >=0%\n";
