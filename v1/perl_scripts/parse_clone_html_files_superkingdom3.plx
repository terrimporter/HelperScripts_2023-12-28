...
open (OUT,">>","clone_summary_superkingdom.txt") || die ("Error cannot write to outfile:$!\n");
			...
			if ($flag==0) {
				...
				$flag=1;
			       ...
			       if ($line =~ /\(superkingdom\)/) {
				       ...
				       if ($line =~/<\/a>/) {
						$flag=2;
						$line =~/>(\w+)<\/a>\s{1}\(superkingdom\)\s{1}(\S+)%/;
						$order = $1;
						$pp = $2;
						if ($pp =~ /~/) {
							$pp =~ s/~//;
...
open (IN,"<","clone_summary_superkingdom.txt") || die ("Error cannot read from clone summary txt: $!\n");
@in = <IN>;
close IN;

open (OUT2,">>","clone_summary_best_superkingdom.txt") || die ("Error cannot write to clone summary best txt: $!\n");
