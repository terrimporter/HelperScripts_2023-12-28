...
while (($gi, $kingdom) = each(%kingdom) ) {	
	$kingdom = $kingdom{$gi};
	$ref_kingdom = $ref_kingdom{$gi};
	if ($kingdom =~ /^\S+/) {
		$coverage++;
...
while (($gi, $class) = each(%class) ) {	
	$ref_class = $ref_class{$gi};
	$class = $class{$gi};
	if ($class =~ /^\S+/) {
		$coverage++;
...
while (($gi, $species) = each(%species) ) {	
	$ref_species = $ref_species{$gi};
	$species = $species{$gi};
	if ($species =~ /^\S+/) {
		$coverage++;
