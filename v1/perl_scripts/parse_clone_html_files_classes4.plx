#!/usr/bin/perl
#Oct.6, 2011 edited to work with gi ids only
...
open (OUT,">>","clone_summary_classes.txt") || die ("Error cannot write to outfile:$!\n");
...
open (IN,"<","clone_summary_classes.txt") || die ("Error cannot read from clone summary txt: $!\n");
@in = <IN>;
close IN;

open (OUT2,">>","clone_summary_best_classes.txt") || die ("Error cannot write to clone summary best txt: $!\n");
...
#		$id_part =~ /\d+\|\*\|(\w{14})/;
		#my @id_part = split(/\|/,$id_part);###edit here to grab gi number###
		$id_current = $id_part;
...
open (IN,"<","clone_summary_best_classes.txt") || die ("Error can't read best class file: $!\n");
@infile = <IN>;
close IN;
...
print "There are $num_keep classes with support >=0%\n";
