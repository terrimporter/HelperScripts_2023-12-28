...
# USAGE perl parse_orfs6.plx mutated_1.aa.fasta 
...
my $accession;
		...
		# 0 - >lcl|ORF6_AF301593.1:387:917 or >lsl|ORF6_AF301593.1_pseudogene:387:917
		# 1 - unnamed
		# 2 - protein
		# 3 - product,
		#4 - partial
		...
		@orfline = split(/:/, $orfline);
			# 0 - >lcl|ORF6_AF301593.1 or >lsl|ORF6_AF301593.1_pseudogene
			# 1 - 387
			# 2 - 917
		$start = $orfline[1];
		$stop = $orfline[2];
		...
		if ($orfacc =~ /pseudogene/) {
			...
			$orf = $orfacc[0];
#			print $orf."\n"; # testing
			$accession = $orfacc[1]."_pseudogene";
		}
		else {
			@orfacc = split(/_/, $orfacc);
			# 0 - ORF6
