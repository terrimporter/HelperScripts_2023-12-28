...
# Accomodate FASTA header names molecule_filter_PCRStep_amplicon_otu
...
my $moleculeFilterPCRStepAmpliconOtu;
		...
		@line = split(/ /, $line); #process headier line
			# 0 - >lcl|molecule_filter_PCRStep_Amplicon_Otu1:2:310
			# 1 - ORF1_Amplicon_Otu1:1:309
		$orfline = $line[1];

		@orfline = split(/:/, $orfline);
			# 0 - ORF1_molecule_filter_PCRStep_Amplicon_Otu1
			# 1 - 1
			# 2 - 309
		$start = $orfline[1];
		$stop = $orfline[2];
		...
		return ($orf, $moleculeFilterPCRStepAmpliconOtu, $length);
 }
...
my $moleculeFilterPCRStepAmpliconOtu;
		...
		# 0 - >lcl|ORF1_molecule_filter_PCRStep_Amplicon_Otu1:2:310
		# 1 - unnamed
		# 2 - protein
		# 3 - product,
		#4 - partial
		...
		@orfline = split(/:/, $orfline);
			# 0 - >lcl|ORF1_molecule_filter_PCRStep_Amplicon_Otu1
			# 1 - 2
			# 2 - 310
		$start = $orfline[1];
		$stop = $orfline[2];
