...
sub fasta_to_agrep_format {
	...
	while (<IN>) {
		...
		chomp $line;
		if ($line =~ /^>/) {
			$line =~ /^>(\w{16})/; ####Edit to work with older 454 IDS#####
			$id = $1;
		}
		elsif ($line =~ /^\w+/) {
			$seq = $line;
...
sub agrep_search {
	...
	while ($primersequence[$i]) {
		...
		@output = qx(agrep -i -1 -D1 -I1 "^$pattern" temp3.txt); #####allow one error only in primer ######
		$fname = $primername[$i];
...
sub sort_qual_entries_by_locus {
	...
	while ($output[$l]) {
		...
		while ($qual[$w]) {
			...
			if ($line =~ /^>/) {
				$header = $line;
				$header =~ /^>(\w{16})/; #####Edit to work with older 454 ids#####
				$id = $1;
...
sub order_fasta_to_qual {
	...
	while ($basenames[$l]) {
		...
		open (QUAL,"<",$qualname) || die ("Error opening sorted qual file: $!\n");
		@qual = <QUAL>;
		close QUAL;
		...
		while ($qual[$i]) {
			...
			if ($line =~ /^>/) {
				...
				while ($fasta[$j]) {
					...
					chomp $line2;
					if ($line2 =~ /^>/) {
						$line2 =~ /^>(\w{16})/; #####edit to work with older 454 ids#####
						$id2 = $1;
						if ($id2 eq $id && $flag==0) {
							$k=$j+1;
							$seq = $fasta[$k];
