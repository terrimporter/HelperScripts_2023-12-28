...
while( ($readid,$val) = each(%first) ) {
	...
	if (exists $second{$readid}) {
		$centroid = $second{$readid};
		
		if (exists $third{$centroid}) {
			$OTUcentroid = $third{$centroid};
			print OUT "$readid\t$OTUcentroid\n";
		}
		else {
			print "problem mapping centroid $centroid to an OTUcentroid in 3rd map file.\n";
		}
	}
	else {
		$centroid = $readid;
...
$result = system('usearch6.0.307_i86linux32 -sortbysize cat_nonch_ref.fa -output cat_sorted.fa');
