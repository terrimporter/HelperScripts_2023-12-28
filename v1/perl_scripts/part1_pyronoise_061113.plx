#!/usr/bin/perl
##June 11, 2013 edited to run pyronoise via mothur
...
my $pwd = '/1/scratch/terri/Shadi_052713/PAD_ms/test/';  ### hard coded, update this as needed
my $grep_term;
my $file_ext;
my $search_term;
my $file_ext2;
...
extract_sff_files(); #edit here to extract fasta, qual, AND flow

print "Use MOTHUR v.1.25.1 to grab first 100-450 bp and run pyronoise.\n";
run_trimflows_shhhflows();

print "Removing new lines from sequence part of fasta sequence files.\n";
$grep_term = ".shhh.fasta";
$file_ext = ".fasta.newlineremoved";
grep_dir1(\$grep_term,\$file_ext);

$output = qx(mkdir trim_flow);
$output = qx(find *.trim.flow -maxdepth 1 -type f -exec mv {} trim_flow \\;);
$output = qx(mkdir scrap_flow);
$output = qx(find *.scrap.flow -maxdepth 1 -type f -exec mv {} scrap_flow \\;);
$output = qx(mkdir flow_files);
$output = qx(find *.flow.files -maxdepth 1 -type f -exec mv {} flow_files \\;);
$output = qx(mkdir flow);
$output = qx(find . -name "*.flow" -maxdepth 1 -type f -exec mv {} flow \\;);
$output = qx(mkdir shhh_names);
$output = qx(find *.shhh.names -maxdepth 1 -type f -exec mv {} shhh_names \\;);
$output = qx(mkdir shhh_counts);
$output = qx(find *.shhh.counts -maxdepth 1 -type f -exec mv {} shhh_counts \\;);
$output = qx(mkdir shhh_groups);
$output = qx(find *.shhh.groups -maxdepth 1 -type f -exec mv {} shhh_groups \\;);
$output = qx(mkdir shhh_fasta);
$output = qx(find *.shhh.fasta -maxdepth 1 -type f -exec mv {} shhh_fasta \\;);
...
$output = qx(find . -name "*.newlineremoved" -maxdepth 1 -type f -exec mv {} fasta_newlineremoved \\;);
...
$output = qx(find . -name "*.agrep" -maxdepth 1 -type f -exec mv {} for_agrep \\;);
$output = qx(mkdir fasta_emptyremoved);
$output = qx(find . -name "*.emptyremoved" -maxdepth 1 -type f -exec mv {} fasta_emptyremoved \\;);
#$output = qx(mkdir extracted_fasta); #did not extract fasta or qual, just flow from sff
...
$output = qx(mkdir shhh_qual);
$output = qx(find *.shhh.qual -maxdepth 1 -type f -exec mv {} shhh_qual \\;);
...
#$output = qx(mkdir extracted_qual); #did not extract fasta or qual, just flow from sff
...
$search_term = ".fastq\$";
$file_ext = ".fastq.trimmed";
$file_ext2 = ".fasta.trimmed";
$file_ext3 = ".fasta.trimmed.stats";
process_by_marker4(\$search_term,\$file_ext,\$file_ext2,\$file_ext3);
...
while ($output[$i]) {
	...
	if ($file =~ /\./) { #contains a dot, to avoid dir names
		@file = split(/\./,$file);
		$basename = $file[0];
		$outfile = $basename.$file_ext;
...
#			print "file:$file\n";
			...
			if ($file !~ /^\./ && $file =~ /$search_term/) {
				...
				$output = qx(cutadapt -b $primer -e 0.1 -m 100 -M 450 -q 20 -O 15 -n 2 $infile > $outfile);
				# -b infile
				# -o outfile
				...
				open (IN, "<", $outfile) || die "Error cannot open outfile from cutadapt: $!\n";
				@in = <IN>;
				close IN;
				...
				close FASTA;
				@in=();
				$outfile2 = $path_dir.$basename.$file_ext3;
				print "Getting trimmed read stats.\n";
				get_stats(\$fastafile,\$outfile2);
				$fastafile=();
...
#	if ($name !~ /^\.+/) {
#		$output = qx(mothur "#sffinfo(sff=$name)");
#	}
#	$i++;
#	$name=();
...
qx(ls | grep sff | parallel -j 4 'mothur "#sffinfo(sff={}, flow=T)"');

}

####################

sub run_trimflows_shhhflows {
...
#	if ($name !~ /^\.+/) {
#		$output = system("mothur '#trim.flows(flow=$name)'");
#	}
#	$i++;
#	$name=();
#}
...
print "\tRunning trim.flows...\n";
qx(ls | grep flow | parallel -j 4 'mothur "#trim.flows(flow={},minflows=100,maxflows=450)"');

print "\tRunning shhh.flows...\n";
qx(ls | grep trim.flow | parallel -j 4 'mothur "#shhh.flows(flow={})"');
...
sub fasta_to_agrep_format {
	...
	while (<IN>) {
		...
		chomp $line;
		if ($line =~ /^>/) {
			$line =~ /^>(\w{14,16})/; #####Edit this to work with old or new 454 IDS#####
			$id = $1;
		}
		elsif ($line =~ /^\w+/) {
			$seq = $line;
