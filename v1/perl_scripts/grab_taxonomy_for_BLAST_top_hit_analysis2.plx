#!/usr/bin/perl
#Dec. 15, 2016 add superkingdom
#November 11, 2016 updated to be used to replicate BLAST top hit analysis
#use after blastn with staxid output
...
while ($taxids[$i]) {
	...
	if ($taxid == -1 ) {
		...
		if (length $kingdom > 0) {
			...
			print OUT "$taxid\t$superkingdom\t$kingdom\t$phylum\t$class\t$order\t$family\t$genus\t$species\n";
#			print "got to first if statement\n";
			...
			if (exists $merged{$taxid}) {
				...
				print OUT "$new_taxid\t$superkingdom\t$kingdom\t$phylum\t$class\t$order\t$family\t$genus\t$species\n";
#				print OUT "$taxid\t$kingdom\t$phylum\t$class\t$order\t$family\t$genus\t$species\n"; OMIT THIS LINE FOR BLAST TOP HIT ANALYSIS
