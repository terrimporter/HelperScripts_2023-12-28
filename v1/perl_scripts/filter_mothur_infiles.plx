...
#usage $perl filter_mothur_infiles.plx
...
@output = qx(ls | grep ".list");
$list_file = $output[0];
chomp $list_file;

print "Opening $list_file to process...\n";
...
print "Removing singletons and doubletons...\n";
...
while ($line[$i]) {
	...
	if ($otu_line =~ /,/) {
		...
		if ($num_reads>=3) {###### modify read/otu cutoff here #####
...
print "Please enter name of .groups file to edit:\n";
$group_file = <STDIN>;
chomp $group_file;
