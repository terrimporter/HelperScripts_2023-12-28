#!/usr/bin/perl
#Oct.6, 2010 edited to work with gi ids only
...
open (OUT,">>","clone_summary_phylum.txt") || die ("Error cannot write to outfile:$!\n");
...
open (IN,"<","clone_summary_phylum.txt") || die ("Error cannot read from clone summary txt: $!\n");
@in = <IN>;
close IN;

open (OUT2,">>","clone_summary_best_phylum.txt") || die ("Error cannot write to clone summary best txt: $!\n");
...
open (IN,"<","clone_summary_best_phylum.txt") || die ("Error can't read best phylum file: $!\n");
@infile = <IN>;
close IN;
...
print "There are $num_keep phylum with support >=0%\n";
