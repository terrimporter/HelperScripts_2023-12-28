...
my $alnBait;
my $ref;
my $minBaits = 100; ### Edit minimum number of aligned baits required to be in file before parsing
my $range;
my $sampleSize = 1; ### Edit number of baits to sample per species
my $outfile;
my $randIndex;
my $random;
my $newid;
...
#		}
		...
		#instead of above, just shuffle, then sample up to the required amount
		@keysShuffled = shuffle(@keys);
...
#			if ($j < $sampleSize) {
			...
			if (exists $aln{$gb}) {
				...
				print "Error in file $file, problem with $ref $gb\n";
			}
