...
print OUT "Sample\tOTUcentroid\tReadid\tTaxonid\tKingdom\tPhylum\tClass\tOrder\tFamily\tGenus\n";
print OUT2 "Sample\tOTUcentroid\tTaxonid\tKingdom\tPhylum\tClass\tOrder\tFamily\tGenus\n";
...
while ($in2[$i]) {
	...
	if ($line =~ /^Sample/) {
		...
		$family = $line[8];
		$genus = $line[9];
		$lineage = $taxonid."\t".$kingdom."\t".$phylum."\t".$class."\t".$order."\t".$family."\t".$genus;
