...
open (OUT,">>","clone_summary_orders.txt") || die ("Error cannot write to outfile:$!\n");
...
open (IN,"<","clone_summary_orders.txt") || die ("Error cannot read from clone summary txt: $!\n");
@in = <IN>;
close IN;

open (OUT2,">>","clone_summary_best_orders.txt") || die ("Error cannot write to clone summary best txt: $!\n");
...
open (IN,"<","clone_summary_best_orders.txt") || die ("Error can't read best order file: $!\n");
@infile = <IN>;
close IN;
...
print "There are $num_keep orders with support >=0%\n";
