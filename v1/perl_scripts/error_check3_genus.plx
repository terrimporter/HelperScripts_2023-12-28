#!/usr/bin/perl
#Nov.10, 2011 edited to do genus comparison only
#NEW USAGE perl error_check3_genus.plx genus.ref genus.error
...
open (GENUS_REF,"<",$ARGV[0]) || die "Error cannot read from genus.ref: $!\n";
@genus_ref = <GENUS_REF>;
close GENUS_REF;
while($genus_ref[$i]) {
	$line = $genus_ref[$i];
...
open (GENUS_ERR,"<",$ARGV[1]) || die "Error cannot read from genus.error: $!\n";
