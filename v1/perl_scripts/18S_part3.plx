...
#modify to work with lepidoptera 18S seqtrim read processing
#
use strict;
use warnings;
...
print "Starting megablast searches against Lepidoptera 18S database\n";
megablast();
...
sub usearch_clustering {
	...
	@output = qx(uclust --cluster sorted.fna --uc results.uc --id 1.0 --nofastalign --rev --usersort --seedsout seed.fasta);
...
sub megablast {
	...
	@output = qx(blastn -task megablast -db /home/terri/blast-2.2.24_32bit/db/18S -query sorted.fna.filtered -out megablast.out -outfmt 6);
