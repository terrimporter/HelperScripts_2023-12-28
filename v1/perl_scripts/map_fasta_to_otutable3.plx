...
#declare arrays
my @otu_files; #original filtered (97% or denoised) OTUs
my @tables; #map original QC'd fastas to the filtered OTUs
my @otufile;
my @tablefile;
my @line;
my @basename;
...
#	$basename = substr $otufile,0,10;
	...
	if (exists $tables{$basename}) {
		...
		while ($otufile[$i]) { #original filtered OTUs (97% or denoised)
			$line = $otufile[$i];
			chomp $line;
			...
			if ($line =~ /^>/) {
				...
				if (exists $otu_size{$otuID}) { #OTU table with OTUid and adjusted OTUsize
					$NEWotusize = $otu_size{$otuID};
					print OUT ">".$otuID."|size=".$NEWotusize."\n";
				}
				else {
					print "Error cannot find basename $basename otu $otuID in table: $!\n";
					print OUT ">".$otuID."|size=0\n";
					#not present in the OTU table because original reads never mapped to this particular filtered OTU, instead mapped to something else first?!?
				}
