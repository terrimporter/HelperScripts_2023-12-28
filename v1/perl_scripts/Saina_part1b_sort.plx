...
#Adjust how the primer file is made, not just on primer name, but also the T123orE123 tag
...
sub agrep_primer_search {
	...
	while ($folders_to_process[$a]) {
		...
		while ($primersequence[$i]) {
			...
			@fname = split(/_/,$fname);
			my $firstpart = $fname[1];
			my $secondpart = $fname[2];
			$folder_name = $firstpart."_".$secondpart;
