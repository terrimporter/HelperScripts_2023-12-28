...
while ($read[$k]) {
	...
	if ($flag==0) {
		...
		if ($line !~ /(REFERENCE|COMMENT)/) {
			...
			elsif ($lineage !~ /$genus_ORGANISM/){
				if ($lineage !~ /symbiont/) {#account for Entylomatales incertae sedis; lineage with Genus Tilletiopsis and species name in ORGANISM field
					print OUT2 "$hit_gi\t$hit_gb\t$lineage $genus_ORGANISM;. $species..\n";
					$hit_gb=();
					$hit_gi=();
					@organism=();
					$lineage=();
				}
				elsif ($genus_ORGANISM =~ /symbiont/) {#accounty for fungal ant symbiont with ant genus and species in ORGANISM field
