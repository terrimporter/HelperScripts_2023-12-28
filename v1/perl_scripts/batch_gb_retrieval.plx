#!/usr/bin/perl
#Sept. 20, 2011 edited to skip over records that contain more than one SEGMENT
#Sept. 19, 2011 problems getting all GB records in one go, so restart with the remaining batch files
#new usage perl batch_gb_retrieval.plx
#
...
#old usage perl ITS_entrez_grab_gb.plx gi.query
...
$factory = Bio::DB::EUtilities -> new (	-eutil	=>	'efetch',
					...
					-rettype=>	'gb',
					-id	=>	\@gi_batch);
$file2 = 'ITS_'.$l.'.genbank';

$factory -> get_Response (	-file	=> $file2);
$seqin = Bio::SeqIO -> new (	-file	=> $file2,
				-format	=> 'genbank');

open (FEATURES,">>","features_4.txt") || die ("Error cannot write to features_4.txt: $!\n");###edit features file number too###
print FEATURES "ID\tOrganism\tITS sequence\n";

while ($seq = $seqin -> next_seq) {
	$id = $seq -> id;
	$annotation = $seq->annotation;
	@annotations = $annotation->get_Annotations('segment');
	$segment = $annotations[0]; #just check first part of segment if it exists
	if ($segment =~ /^\S+/) { #if segment exists and has a value, then print nil and move to next record
		$organism = "nil";
		$spacer1 = "nil";
		$ITS_seq = "nil";
		$spacer2 = "nil";
