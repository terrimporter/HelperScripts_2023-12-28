...
#modify to exclude same accession if it's the top hit
#modified to prompt for arguments
#modified regex to grab query = gi | acc
#
use strict;
use warnings;
...
while ($blast[$i]) {
	...
	if ($flag==0) {
		if ($line =~ /^Query\=/) {
			$line =~ /Query\=\s+\w+\|(\w+)/g;###edit regex here###
			$query = $1;
			push(@query,$query);
			$flag=1;
