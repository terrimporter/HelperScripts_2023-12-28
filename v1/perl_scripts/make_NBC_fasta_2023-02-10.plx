#!/usr/bin/perl
# Feb. 10/23 remove the < and > symbols from taxon names; make sure species have a binomial name
...
# temp file of rejected seqs to look over
open (OUT2, ">>", "junk.txt") || die "Error cannot open junk outfile\n";
...
while ($lineage[$i]) {
	...
	if ($line =~ /^\S+/) {
		...
		if ($superkingdom eq '') {
			...
			print OUT2 "No superkingdom\t".$line."\n";
			$i++;
			$line=();
			@line=();
			$tax=();
		...
		if ($species eq '') {
			...
			$lineage =~ s/\<//g; # this punctuation messes up the classifier
			$lineage =~ s/\>//g;
			$lineage{$tax} = $lineage;
		}
		elsif ($species =~ /(sp\.|nr\.|aff\.|spp\.|cf\.)/) { #only keep good species binomials and BOLD BINs
			$lineage{$tax} = "undef;undef;undef;undef;undef;undef;undef;undef;OMITTHISONE";
			print OUT2 "No species insuff\t".$line."\n";
			...
			$lineage =~ s/\<//g; # this punctuation messes up the classifier
			$lineage =~ s/\>//g;
			@species_split = split(/_/,$species); # exclude taxa that don' thave a binomial species name
			$num = scalar(@species_split);
			if ($num == 2) {
				$lineage{$tax} = $lineage;
			}
			else {
				$lineage{$tax} = "undef;undef;undef;undef;undef;undef;undef;undef;OMITTHISONE";
				print OUT2 "No species words\t".$line."\n";
