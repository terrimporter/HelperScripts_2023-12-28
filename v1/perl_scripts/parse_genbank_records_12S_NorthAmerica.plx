#!/usr/bin/perl
# March 9, 2023 track country
# July 26, 2021 based on quick_filter_gb_results_length5b.plx
# Script to parse through genbank records using Bio::SeqIO
# USAGE perl parse_genbank_records_12S.plx
...
while ($files[$i]) {
	...
	if ($file =~ /^\./) {
		...
		open (GB_COUNTRY, ">>", "gb_country.map") || die "Error cannot write to gb_country.map: $!\n";
    				...
    				foreach my $value ( @annotations ) {
						...
						if ($lower =~ /^([A-Z]+)([0-9]+)$/ ) {
							...
							if ($upper =~ /^([A-Z]+)([0-9]+)$/) {
								...
								$outfile = submit_entrez_query(\@arr2,$range);
		...
		if ($feat_object -> primary_tag eq "source") {
			...
			if ($feat_object -> has_tag('country')) {
				for $value ($feat_object -> get_tag_values('country')){
					$country = $value;
					print GB_COUNTRY "$gb\t$country\n";
				}
			}
		}
		if ($feat_object -> primary_tag eq "rRNA") {
			$sequence_fragment = $feat_object -> spliced_seq -> seq; #grab 12S seq
			if ($feat_object -> has_tag('product')) {
				@value_gene = $feat_object -> get_tag_values('product');
				foreach $value_gene (@value_gene) {
					if ($value_gene =~ /^12S ribosomal RNA$/i) { #double check product
