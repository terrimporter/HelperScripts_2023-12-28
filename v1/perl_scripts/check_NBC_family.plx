#!/usr/bin/perl
#April 29, 2013 to check family rank
#March 25, 2013 by Terri Porter
#Script to check genus rank classification tests using subusets of testNBC.fasta classified using testNBC.taxonomy
...
while ($nbc[$i]) {
	...
	@line = split(/\t/, $line);
	$scalar = scalar(@line);
	$genusField = $scalar-6;#edited to grab family asst
	$bootstrapField = $scalar-4; #edited to grab family bs
	$id = $line[0];
...
while ($known[$i]) {
	...
	if ($line =~ /^>/) {
		...
		$id = $1;
		$scalar = scalar(@line);
		$max = $scalar-2;#edit to grab family
		$known = $line[$max];
