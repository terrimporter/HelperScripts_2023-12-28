#!/usr/bin/perl
#Oct.16,2013 edit to run FASTER!!!
...
#declare hash
my %reads_to_remove_from_group_file; #index by readid, value = 1
...
while ($line[$i]) {
	...
	if ($otu_line =~ /,/) {
		...
		if ($num_reads>=2) {###### modify read/otu cutoff here #####
			...
			foreach $x (@otu_line) {
				$reads_to_remove_from_group_file{$x} = 1;	
#				push(@reads_to_remove_from_group_file, $x)
			}
		}
	}
	else {
		$reads_to_remove_from_group_file{$otu_line} = 1;
...
$new_list_file = $list_file.".filtered_test";
...
while (<GROUP>) {
	...
	if (exists $reads_to_remove_from_group_file{$readid}) {
		$line=();
		@line=();
		$readid=();
		$sample=();
...
$group_file_filtered = $group_file.".filtered_test";
