#!/usr/bin/perl
#July 11, 2013 edited to work with lepidoptera and diptera BOLD fastas too
...
#	$string = $order."|".$orderVal."|".$family."|".$familyVal."|".$genus."|".$genusVal;
	$string = $order."|".$orderVal."|".$family."|".$familyVal;
#	print "string:$string\n";
	$asst{$id} = $string;
...
#		$genusVal = $string[5];
		...
		if ($orderMorph eq $order && $orderVal >= 0.80) {
				$match++;
...
#			}
		...
		elsif ($orderMorph ne $order && $orderVal >= 0.80) {
			$mismatch++;
...
print "There were $match matches between the morphid and assigned order (60%+)\n";
print "There were $mismatch mismatches between the morphid and assigned order (60%+)\n";
#print "$genus_improved assignments were improved to the genus rank with nbc (70%+)\n";
print "$family_improved assignments were improved to the family rank with nbc (80%+)\n";
