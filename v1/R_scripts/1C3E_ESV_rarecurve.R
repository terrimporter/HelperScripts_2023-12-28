library(vegan)
pdf("1C3E_rarecurve.pdf")
par(mfrow=c(1,1), mar=c(7,4,4,2))

#indvdenoised_97OTUs
A<-read.csv(file="1C3E_OTU_matrix.csv", head=TRUE, row.names=1)
#A
...
rarecurve_ESV<-function (x, step, sample,main="2016 Grid 1C3E Expt",xlab="Reads", ylab="ESVs",label=TRUE, col, lty, ...){
	x <- as.matrix(x)
